...
export const Images = styled.div<{ active?: boolean }>`
  display: grid;
...
export const Hour = styled.div`
  ...
  color: #5c8599;
`;
export const OpenOnWeekends = styled.div<{ open?: boolean }>`
  background: ${(props) =>
    props.open
      ? 'linear-gradient(154.16deg, #fdf0f5 7.85%, #FFFFFF 91.03%)'
      : 'linear-gradient(154.16deg, #edfff6 7.85%, #ffffff 91.03%)'};
  border: 1px solid ${(props) => (props.open ? '#ffbcd4' : '#a1e9c5')};
  color: ${(props) => (props.open ? '#ff669d' : '#37c77f')};
`;
